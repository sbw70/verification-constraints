...
SPENT_STATE_PATH = Path("/home/seth/wp2_t1_spent_state_archer.json")
CRASH_STAGE_PATH = Path("/home/seth/wp2_t1_crash_stage.txt")
...
def atomic_write_json(path, obj):
    ...
    maybe_crash("after_temp_fsync_before_replace")
...
def maybe_crash(stage):
    try:
        configured = CRASH_STAGE_PATH.read_text(
            encoding="utf-8"
        ).strip()
    ...
    CRASH_STAGE_PATH.unlink()

    print(
        "POC006_CONTROLLED_SIGKILL stage={}".format(stage),
        flush=True,
    )
...
def deny(reason, verified=False, artifact_id=None):
    return {
        ...
        "boundary": "wp2_t1_pre_replace_pi",
...
def spend(package, spend_request):
    ...
    with STATE_LOCK:
        ...
        maybe_crash("after_persist_before_accept")
...
def main():
    ...
    print("WP2_T1_PRE_REPLACE_PI_BOUNDARY")
